...
if __name__ == '__main__':
    ...
    print("\ngrayscale sensor for line folloing and ultrasonic sensor for breaks.")
    car = Picarx()
    ...
    grayscale_control = ConsumerProducer(interpreter.process,input_busses=gray_in_bus,output_busses=gray_out_bus,delay=1.0,
                                         name="grayscale_control")
    grayscale_consumer = Consumer(controller.steer,input_busses=gray_out_bus,delay=1.0,name="grayscale_consumer")

    
    
    ultrasonic_producer = Producer(ultrasonic.read,
                                   output_busses=ultrasonic_input, delay=0.3,
                                   name="ultrasonic_producer")

    ultrasonic_control = ConsumerProducer(ultrasonic.process,
                                          input_busses=ultrasonic_input,
                                          output_busses=ultrasonic_output, delay=0.5,
                                          name="ultrasonic_control")
    ultrasonic_consumer = Consumer(ultrasonic.control,
                                   input_busses=ultrasonic_output, delay=0.5,
