...
class ArmMove():
    ...
    def recall(self):
        ##multiple grasping attempts failed so here we are
        global start_pick_up
        print("Mission abort")
    ...
    def sonar_sensor(self):
        ...
        print("calling ultrasonic sensor")
        time.sleep(2)
    ...
    #Picks up the cube
    def pickUp(self, adder=0):
        global pos
        world_X = pos[0]
        world_Y = pos[1]
        rotation_angle = pos[2]
        time.sleep(0.5)
        servo2_angle = getAngle(world_X, world_Y, rotation_angle) + adder
        ...
        while True:    
            ...
            if detect_color != 'None' and start:  #If it detects that the block has not moved for a while, start gripping 
                ...
                if result == False:
                    ...
                    dis = self.sonar_sensor()
                    print("Found distance: ", dis)
                    adder = 270
                    while dis > 15:
                        self.pickUp(adder)
                        dis = self.sonar_sensor()
                        ...
                        if adder < 0:
                            dis = 0
                            print("failed grasping all attempts")
                            self.recall()
                            #continue
