        ...
        if __isRunning:        
            if detect_color != 'None' and start_pick_up:  #如果检测到方块没有移动一段时间后，开始夹取
                ...
                if result == False:
                    ...
                    Board.setBusServoPulse(1, servo1 - 280, 500)  # open
                    ...
                    if distance > 8:
                        ...
                        print("####### Oops grasp failed  ########")
                        #servo2_angle = getAngle(world_X, world_Y, rotation_angle)#计算夹持器需要旋转的角度
                        print("### reattempting ####")
                        time.sleep(2)
                        Board.setBusServoPulse(1, servo1-200, 500)  # 爪子张开
                        time.sleep(2)
                        Board.setBusServoPulse(2, servo2_angle+270, 500)
                        time.sleep(2)
                        if not __isRunning:
                            continue
                        AK.setPitchRangeMoving((world_X, world_Y, 1.5), -90, -90, 0, 1000)
